...
exports.ChakravyuhHttpClient = void 0;
...
class ChakravyuhHttpClient {
    ...
    constructor(config) {
        this._apiKey = new auth_1.ApiKey(config.apiKey);
        ...
        for (let attempt = 0; attempt <= this._maxRetries; attempt++) {
            try {
                ...
                if (!response.ok) {
                    ...
                    const error = (0, errors_1.errorFromResponse)(response.status, parsed, retryAfter);
                ...
                try {
                    ...
                    throw new errors_1.NetworkError(`Failed to decode response JSON from ${opts.method} ${opts.path}`, false);
                }
                return data;
            }
            catch (err) {
                // NetworkError / TimeoutError → retry if appropriate
                if (err instanceof errors_1.NetworkError && err.retryable && attempt < this._maxRetries) {
                    const delay = this._retryBaseDelay * Math.pow(2, attempt) + jitter();
                    await sleep(delay);
                    lastError = err;
                    continue;
                ...
                if (err instanceof Error &&
                    ...
                    if (err.name === "AbortError") {
                        throw new errors_1.NetworkError(`Request timed out after ${this._timeout}ms`, true);
                    }
                    throw err;
                }
                // Unknown error → wrap in NetworkError
                throw new errors_1.NetworkError(err instanceof Error ? err.message : String(err), true);
            }
        }
        // Should not reach here, but just in case
        throw lastError instanceof Error
            ? lastError
            : new errors_1.NetworkError("Max retries exceeded", false);
    }
    // ─── Header construction ─────────────────────────────────────
    buildHeaders(opts) {
        const headers = {
...
exports.ChakravyuhHttpClient = ChakravyuhHttpClient;
// ─── Helpers ────────────────────────────────────────────────────
