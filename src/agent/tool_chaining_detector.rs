...
fn default_patterns_enabled() -> bool {
    true
}
...
impl Default for ToolChainingDetectorConfig {
    fn default() -> Self {
        Self {
            ...
            default_patterns_enabled: default_patterns_enabled(),
            custom_patterns: vec![],
        }
    }
}
...
fn default_dangerous_chains() -> Vec<ChainPattern> {
    vec![
        ChainPattern {
            ...
            risk_weight: 8.0,
            description: "Reading files then emailing them — potential data exfiltration".into(),
        },
        ChainPattern {
            name: "C2 Communication".into(),
            sequence: vec!["code_exec".into(), "network_call".into()],
            risk_weight: 9.0,
            description: "Executing code then making network calls — potential C2".into(),
        },
        ChainPattern {
            name: "Database Theft".into(),
            sequence: vec!["db_dump".into(), "external_transmit".into()],
            risk_weight: 9.0,
            description: "Dumping database then transmitting externally".into(),
        },
        ChainPattern {
            name: "Memory Exfiltration".into(),
            sequence: vec!["memory_read".into(), "api_call".into()],
            risk_weight: 7.0,
            description: "Reading memory then making API calls — data exfiltration".into(),
        },
        ChainPattern {
            name: "Persistence Install".into(),
            sequence: vec!["file_write".into(), "execute".into()],
            risk_weight: 7.0,
            description: "Writing then executing — persistence".into(),
        },
        ChainPattern {
            name: "Reverse Shell".into(),
            sequence: vec!["shell_access".into(), "network_call".into()],
            risk_weight: 9.0,
...
fn normalize_tool(tool: &str) -> String {
    ...
    if t.contains("email_send") || t.contains("send_email") {
        return "email_send".into();
    }
    if t.contains("code_exec") || t.contains("execute_code") {
        return "code_exec".into();
    }
    if t.contains("network_call") || t.contains("http_request") || t.contains("fetch") {
        return "network_call".into();
    }
    if t.contains("db_dump") || t.contains("sql_dump") {
        return "db_dump".into();
    }
    if t.contains("external_transmit") || t.contains("upload") {
        return "external_transmit".into();
    }
    if t.contains("memory_read") || t.contains("rag_query") {
        return "memory_read".into();
    }
    if t.contains("api_call") || t.contains("rest") {
        return "api_call".into();
    }
    if t.contains("file_write") || t.contains("create_file") || t.contains("save") {
        return "file_write".into();
    }
    if t.contains("execute") || t.contains("run") {
        return "execute".into();
    }
    ...
    pub fn evaluate(&self, tools: &[String]) -> ChainRisk {
        if !self.config.enabled {
            return ChainRisk {
                ...
                summary: "tool chaining detector disabled".into(),
            };
        }
