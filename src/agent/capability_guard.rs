...
fn tool_to_capability(tool: &str) -> Vec<Capability> {
    ...
    if t.contains("file_delete") || t.contains("remove") {
        caps.push(Capability::FileDelete);
    }
    if t.contains("http") || t.contains("curl") || t.contains("fetch") || t.contains("network") {
        caps.push(Capability::NetworkAccess);
    }
    if t.contains("api_call") || t.contains("rest") {
        caps.push(Capability::ApiCalls);
    }
    if t.contains("memory") || t.contains("rag") {
        caps.push(Capability::MemoryAccess);
    }
    if t.contains("shell") || t.contains("bash") || t.contains("cmd") {
        caps.push(Capability::ShellAccess);
    }
    if t.contains("email") || t.contains("smtp") {
        caps.push(Capability::EmailSend);
    }
    if t.contains("sql") || t.contains("db") || t.contains("database") {
        caps.push(Capability::DatabaseAccess);
    }
    caps
}
...
impl CapabilityGuard {
    pub fn new(config: &CapabilityGuardConfig) -> Self {
        Self {
            config: config.clone(),
        }
    }
    ...
    pub fn evaluate(&self, agent_type: &AgentType, tools: &[String]) -> CapabilityGuardResult {
        ...
        let reason = if allowed {
            ...
            format!("denied capabilities: {}", denied.join(", "))
        };
